...
from JExtract require import Array2 Array4 Array5.
from JExtract require import WArray128 WArray160.
...
module OpsV = {
  ...
  proc iVPSRL_4u64( x: vt4u64, p: W8.t): vt4u64 = {
    return VPSRL_4u64 x p;
  }
...
proof.
  ...
  have -> : (to_uint (pack4 [t{1}.[0]; t{1}.[1]; t{1}.[2]; t{1}.[3]]) %/ 2^i %% 2 <> 0) = 
...
proof.
  ...
  move: (W4u64.Pack.init_of_list (fun i => vv{1}.[k].[i])).
  rewrite /= -iotaredE /= => <-.
  move: (W4u64.Pack.init_of_list (fun j => vv{1}.[i{2} <- vv{1}.[i{2}].[o{2} <- v{2}]].[k].[j])).
  rewrite /= -iotaredE /= => <-.
...
proof.
  ...
  rewrite get8_pack4u64.
  move: (W4u64.Pack.init_of_list (fun j => vv{1}.[i{2}].[j])).
  rewrite /= -iotaredE /= => <- /=.
  rewrite ifT 1:/# /= divzMDl 1:// divz_small 1:// modzMDl /=.
  by rewrite initiE 1:// modz_small 1:// unpack8E initiE //.
qed.
...
proof.
  ...
  have ? : 0 <= o{2} * 8 + j < `|32| by smt().
  rewrite modzMDr divzMDr 1:// divz_small 1:// modz_small 1:// /=.
  rewrite unpack8E // initiE 1:// /= get8_pack4u64 ifT 1:/#.
  move: (W4u64.Pack.init_of_list (fun j => vv{1}.[i{2}].[j])).
  rewrite /= -iotaredE /= => <-.
  by rewrite divzMDl 1:// divz_small 1:// modzMDl /= initiE 1:// modz_small 1:// /#.
qed.

equiv eq_iVPBROADCAST_4u64 : Ops.iVPBROADCAST_4u64 ~ OpsV.iVPBROADCAST_4u64 : ={v} ==> is4u64 res{1} res{2}.
proof.
proc; auto => /> &1 &2.
by rewrite /is4u64 /VPBROADCAST_4u64 -iotaredE.
qed.
...
proof. 
  ...
  move: (W4u64.Pack.init_of_list (fun j => loadW64 mem{2} (to_uint (p{2} + W64.of_int (8 * j))))); rewrite /= -iotaredE /= => <-.
  have ? : 0 <= j %/ 8 < 4 by rewrite ltz_divLR // lez_divRL.
  have ? := modz_cmp j 8.
