(******************************************************************************
   Keccak_f1600_s_ref_proof.ec:

   Correctness proof for the REF implementation
******************************************************************************)
...
hoare round_ref_h _a _c:
 M.__round_ref :
  a = _a /\ rc = _c ==> res = keccak_round_op _c _a.
proof.
proc; simplify.
while (0 <= y <= 5 /\ #pre /\
 ...
 wp; ecall (set_row_ref_h a e b y rc).
...
while (2 <= to_uint c <= 24 /\ 2 %| to_uint c /\
 ...
 wp; ecall (round_ref_h e rc).
 wp; ecall (round_ref_h a rc).
